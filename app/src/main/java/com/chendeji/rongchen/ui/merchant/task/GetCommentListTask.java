...
//    }
    ...
    @Override
    protected void fromDBDataSuccess(ReturnMes<List<Comment>> listReturnMes) {

    }

    @Override
    protected void fromNetWorkDataSuccess(ReturnMes<List<Comment>> listReturnMes) {
        if (AppConst.OK.equals(listReturnMes.status)) {
            mTaskCallBack.onPostExecute(listReturnMes);
        } else {
            ErrorInfo errorInfo = listReturnMes.errorInfo;
    ...
    @Override
    protected ReturnMes<List<Comment>> getDataFromNetwork() throws IOException, HttpException {
